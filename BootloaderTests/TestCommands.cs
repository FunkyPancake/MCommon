...
public class TestCommands {
    ...
    public async void Test_Execute() {
        ...
        iTpMock.Setup(x => x.WriteAsync(It.IsAny<byte[]>()));
    ...
    public async void Test_Reset() {
        ...
        var loggerMock = MockLogger();
        var commands = new Commands(loggerMock.Object, iTpMock.Object);
        var response = await commands.Reset();
        Assert.Equal(ResponseCode.Success, response);

        iTpMock.Verify(m => m.WriteAsync(Match.Create(isThreeCharsLong)));
        iTpMock.Verify(m => m.WriteAsync(Match.Create(isThreeCharsLong)));
    }
    ...
    public async void Test_FlashSecurityDisable() {
        ...
        var loggerMock = MockLogger();
        var commands = new Commands(loggerMock.Object, iTpMock.Object);
        var response = await commands.FLashSecurityDisable(key);
        Assert.Equal(ResponseCode.Success, response);
